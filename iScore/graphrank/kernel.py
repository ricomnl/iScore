...
try:
    ...
    from pycuda import compiler, driver, gpuarray, tools
...
class Kernel(object):
    def __init__(
        ...
        # the cuda kernel
        self.kernel = (
            os.path.dirname(os.path.abspath(__file__)) + "/cuda/cuda_kernel.cu"
        )
    ...
    def import_from_mat(self, mpi_rank=0, mpi_size=1):
        ...
        if self.train_archive is not None:
            ...
            for name in train_names:
                ...
                if g.num_nodes > 0:
                    ...
                    self.max_num_edges_train = np.max(
                        [self.max_num_edges_train, self.train_graphs[name].num_edges]
                    )
                else:
                    print(
                        "Warning : Graph %s was exclued from the training data (num_ndodes = 0)"
                        % name
                    )
        ...
        for name in test_names:
            ...
            if g.num_nodes > 0:
                ...
                self.max_num_edges_test = np.max(
                    [self.max_num_edges_test, self.test_graphs[name].num_edges]
                )
            else:
                print(
                    "Warning : Graph %s was exclued from the testing data (num_ndodes = 0)"
                    % name
                )
        ...
        if filename is None:
            ...
            with open(filename) as f:
                required_names = tuple(
                    [name.split()[0] + "." for name in f.readlines() if name.split()]
                )
            ...
            for (name2, G2) in self.train_graphs.items():
                ...
                if self.debug:
                    ...
                    print(
                        "K      :  "
                        + "  ".join(
                            list(map(lambda x: "{:1.3}".format(x), K[(n1, n2)]))
                        )
                    )
                    ...
                    if Kcheck.ndim > 1:
                        print(
                            ...
                            + "  ".join(
                                list(
                                    map(
                                        lambda x: "{:1.3}".format(x),
                                        Kcheck[ind_test][ind_train],
                                    )
                                )
                            )
                        )
                        ...
                        print(
                            "Kcheck :  "
                            + "  ".join(list(map(lambda x: "{:1.3}".format(x), Kcheck)))
                        )
                        kc = Kcheck
    ...
    def compute_kron_mat(self, g1, g2):
        ...
        pssm1 = np.vstack(
            (g1.edges_pssm, np.hstack((g1.edges_pssm[:, 20:], g1.edges_pssm[:, :20])))
        )
        pssm2 = g2.edges_pssm
        ...
        if self.method == "iter":
            ...
            weight = np.array(
                [
                    self._rbf_kernel(p[0], p[1])
                    for p in itertools.product(*[pssm1, pssm2])
                ]
            )
            ind = np.array(
                [
                    self._get_index(k[0], k[1], g2.num_nodes)
        ...
        if _manual_transpose_:
            ...
            # create the matrix
            self.Wx = sp_sparse.coo_matrix(
                (weight, index), shape=(n_nodes_prod, n_nodes_prod)
            )
    ...
    def compute_px(self, g1, g2, cutoff=0.5):
        ...
        self.px = [
            t[0] * t[1] if (float(t[0]) > cutoff or float(t[1]) > cutoff) else 0
            for t in itertools.product(*[g1.nodes_info_data, g2.nodes_info_data])
        ]
    ...
    def compute_W0(self, g1, g2, method="vect"):
        ...
        if self.method == "iter":
            self.W0 = np.array(
                [
                    ...
                    for p in itertools.product(
                        *[g1.nodes_pssm_data, g2.nodes_pssm_data]
                    )
                ]
            )
            ...
            self.W0 = self._rbf_kernel_vectorized(
                g1.nodes_pssm_data, g2.nodes_pssm_data
            )
    ...
    def _rbf_kernel_vectorized(data1, data2, sigma=10):
        ...
        """
        delta = (
            -2 * np.dot(data1, data2.T)
            + np.sum(data1**2, axis=1)[:, None]
            + np.sum(data2**2, axis=1)
        )
        beta = 2 * sigma**2
    ...
    def compute_kron_mat_cuda(
        self, g1, g2, kernel_name="create_kron_mat", gpu_block=None
    ):  # pragma: no cover
        ...
        if self.debug:
            print(
                "GPU - Mem  : %f \t (block size:%dx%d)"
                % (time() - t0, block[0], block[1])
            )
    ...
    def compute_px_cuda(self, g1, g2, gpu_block=None):  # pragma: no cover
        ...
        create_p_vect(
            info1, info2, pvect, g1.num_nodes, g2.num_nodes, block=block, grid=grid
        )
        self.px = pvect.get()
        driver.Context.synchronize()
        if self.debug:
            print(
                "GPU - Px   : %f \t (block size:%dx%d)"
                % (time() - t0, block[0], block[1])
            )
    ...
    def compute_W0_cuda(self, g1, g2, gpu_block=None):  # pragma: no cover
        ...
        if self.debug:
            print(
                "GPU - W0   : %f \t (block size:%dx%d)"
                % (time() - t0, block[0], block[1])
            )

    def tune_kernel(
        self, func="create_kron_mat", test_all_func=False
    ):  # pragma: no cover
