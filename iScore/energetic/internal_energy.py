...
class InternalEnergy(object):
    ...
    def get_contact_atoms(self):
        ...
        for i, x0 in enumerate(xyz1):
            ...
            contacts = np.where(
                np.sqrt(np.sum((xyz2 - x0) ** 2, 1)) < self.contact_distance
            )[0]
    ...
    def evaluate_charges(self, extend_contact_to_residue=False):
        ...
        if len(charge_data_xyz) == 0:
            charge_data_xyz[tuple([0, 0.0, 0.0, 0.0])] = [0.0]
            charge_data_xyz[tuple([1, 0.0, 0.0, 0.0])] = [0.0]
    ...
    def evaluate_pair_interaction(
        self, print_interactions=False, save_interactions=False
    ):
        ...
        if len(electro_data_xyz) == 0:
            electro_data_xyz0[tuple([0, 0.0, 0.0, 0.0])] = [0.0]
            electro_data_xyz0[tuple([1, 0.0, 0.0, 0.0])] = [0.0]

        if len(vdw_data_xyz) == 0:
            vdw_data_xyz0[tuple([0, 0.0, 0.0, 0.0])] = [0.0]
            vdw_data_xyz0[tuple([1, 0.0, 0.0, 0.0])] = [0.0]
