...
class TestRank(unittest.TestCase):
    ...
    def test1_svmtrain(self):
        ...
        iscore_graph(
            pdb_path=self.train_pdb, pssm_path=self.train_pssm, outdir=self.train_graph
        )
        ...
        iscore_kernel(
            train_graph=self.train_graph, test_graph=self.train_graph, outfile=kfile
        )
    ...
    def test2_svmtest(self):
        ...
        iscore_graph(
            pdb_path=self.test_pdb, pssm_path=self.test_pssm, outdir=self.test_graph
        )
        ...
        iscore_kernel(
            test_graph=self.test_graph, train_archive=self.train_archive, outfile=kfile
        )
