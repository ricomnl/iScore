...
class TestGraph(unittest.TestCase):
    ...
    def test_aligned(self):
        self.assert_graph(
            self.a_pdb, self.a_pssm, self.a_pkl, self.a_check, isaligned=True
        )

    def test_notalligned(self):
        self.assert_graph(
            self.na_pdb, self.na_pssm, self.na_pkl, self.na_check, isaligned=False
        )

    @unittest.expectedFailure
    def test_notalligned_fail(self):
        self.assert_graph(
            self.na_pdb, self.na_pssm, self.na_pkl, self.na_check, isaligned=True
        )
    ...
    def setUp(self):
        ...
        self.na_pssm = {
            "A": "./graph_notalligned/1ATN.A.pssm",
            "B": "./graph_notalligned/1ATN.B.pssm",
        }
